  ...
  public class TransitionScreen : Microsoft.Xna.Framework.DrawableGameComponent
  {
    ...
    void DrawSpinningSquaresTransition(float mu, float alpha)
    {
      ...
      for (int x = 0; x < 8; x++)
      {
        for (int y = 0; y < 4; y++)
        {
            Rectangle rect = new Rectangle(SpriteBatch.GraphicsDevice.Viewport.Width * x / 8, SpriteBatch.GraphicsDevice.Viewport.Height * y / 4, SpriteBatch.GraphicsDevice.Viewport.Width / 8, SpriteBatch.GraphicsDevice.Viewport.Height/ 4);

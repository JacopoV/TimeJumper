...
namespace ParticleSample
{
    ...
    public abstract class ParticleSystem : DrawableGameComponent
    {
        ...
        public void AddParticles(Vector2 where)
        {
            ...
            int numParticles = 
                rangen.Next(minNumParticles, maxNumParticles);
        ...
        protected virtual void InitializeParticle(Particle p, Vector2 where)
        {
            ...
            float velocity =
                minInitialSpeed + (maxInitialSpeed - minInitialSpeed)* (float)rangen.NextDouble();
            float acceleration =
                minAcceleration + (maxAcceleration - minAcceleration) * (float)rangen.NextDouble();
            float lifetime =
                minLifetime + (maxLifetime - minLifetime) * (float)rangen.NextDouble();
            float scale =
                minScale + (maxScale - minScale) * (float)rangen.NextDouble();
            float rotationSpeed =
                minRotationSpeed + (maxRotationSpeed - minRotationSpeed) * (float)rangen.NextDouble();
        ...
        {
            float angle = rangen.Next()* MathHelper.TwoPi;
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            // initializeConstants
			(game as MyPlatformGame.MainGame).spriteBatch.Begin(SpriteSortMode.Deferred, blendState);
            ...
            foreach (Particle p in particles)
            {
                ...
                (game as MyPlatformGame.MainGame).spriteBatch.Draw(texture, p.Position, null, color,
                    p.Rotation, origin, scale, SpriteEffects.None, 0.0f);
            }

            (game as MyPlatformGame.MainGame).spriteBatch.End();
